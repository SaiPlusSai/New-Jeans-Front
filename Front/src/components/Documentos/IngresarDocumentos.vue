...
const registrarDocumento = async () => {
  ...
  loading.value = true
  try {
    const response = await axios.post('/api/documentos/auto', documento.value, {
      headers: { Authorization: `Bearer ${token}` }
    })
...
const actualizarDocumento = async () => {
  ...
  try {
    ...
    await axios.put(`/api/documentos/${documento.value.codigo}`, datosActualizacion, {
      headers: { Authorization: `Bearer ${token}` }
    })
...
const eliminarDocumento = async () => {
  try {
    await axios.delete(`/api/documentos/${documento.value.codigo}`, {
      headers: { Authorization: `Bearer ${token}` }
    })
...
const restaurarDocumento = async () => {
  try {
    await axios.patch(`/api/documentos/${documento.value.codigo}/restaurar`, {}, {
      headers: { Authorization: `Bearer ${token}` }
    })
